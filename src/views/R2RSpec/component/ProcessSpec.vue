<template>
    <div>
      <BaseTable :tableOptions="{...tableOptions,exportTitle:$t(title)+ ' - '+$t('v_r2rspec_process_spec')}" @handleTableChange="handleTableChange" ref="tableRef"/>
      <el-dialog
      ...
      >
        ...
        <ViewHistory v-if="showHistory" ref="viewHistory" :timeArr="timeArr" :switchArr="switchArr" :columns="tableOptions.columns" api="PROCSPECPHOTOHIST_MANAGEMENT#select" />
...
export default {
  ...
  data() {
    return {
      comAddPara,
      GRID_ID:'PROCESS_SPEC',
      specialkeyArr:[],
      timeArr:[],
      switchArr:[],
      numberArr:[],//number类型集合
      tableOptions: {
        id: "PROC_SPEC_PHOTO_KEY",
        columns: [],
        data: [],
        toolbarConfig:{
  ...
  methods: {
    ...
    //加载列表数据
    async load(params = this.searchParams,type='PROCSPECPHOTO_MANAGEMENT#select',isUpdateSignalID){
      objValueIsNull(params)
      const { modelParaList } = this.$api.parameter
      const res  = await modelParaList(params,type)
      if(type !== 'PROCSPECPHOTO_MANAGEMENT#select'){
    ...
    cancelConfirm(){
      ...
      this.selectData = this.selectData.filter(v=>this.defaultSelectData.find(m=>v.ID === m.ID))
    },
    //编辑
    @getHasPrivTip('U')
    ...
    async toSave(data){
      ...
      this.load(filterData, filterData[0].tableType === 'add' ? 'R2RMODEL_MANAGEMENT#save' : 'PROCSPECPHOTO_MANAGEMENT#update',data !== undefined ? this.tableOptions.data[data][this.tableOptions.id] : data)
    },
    // 历史
