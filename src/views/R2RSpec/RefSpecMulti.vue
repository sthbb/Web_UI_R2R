...
export default {
  ...
  data() {
    return {
      ...
      searchForm:[],
      searchParams:{
        R2R_MODEL_NAME:'REF_SPEC'
      },
      FUNC_ID :"",
      GRID_ID :"REF_SPEC_MULTI",
      isCollapse: false,
      specialkeyArr:[],
      timeArr:[],
      timeFormat:'',
  ...
  methods: {
    @getFuncID('R2RSpec/RefSpecMulti')
    init(){
    ...
    async searchFn(data,type){
      type === 1 ? searchFn.call(this,data) : resetSubmit.call(this,{...data,R2R_MODEL_NAME:'REF_SPEC'})
    },
    //搜索
    @getHasPrivTip('R')
    @searchValidata()
    ...
    @getHasPrivTip('C')
    async add(){
      await comInitUnimport.call(this,'REF_SPEC_MULTI_POP_ADD_PARA')
