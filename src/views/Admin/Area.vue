...
export default {
  ...
  methods: {
    ...
    //加载列表数据
    async load(params = this.searchParams,type='AREA_MANAGEMENT#select',isUpdateSignalID){
      objValueIsNull(params)
      const { modelParaList } = this.$api.parameter
      const res  = await modelParaList(params,type)
      if(type !== 'AREA_MANAGEMENT#select'){
    ...
    async toDel(data) {
      ...
      this.load(delIds, 'AREA_MANAGEMENT#delete')
    },
    // 保存
    @saveValidate()
    async toSave(data){
      ...
      this.load(filterData, filterData[0].tableType === 'add' ? 'AREA_MANAGEMENT#save' : 'AREA_MANAGEMENT#update',data !== undefined ? this.tableOptions.data[data][this.tableOptions.id] : data)
    }
  }
}
</script>
