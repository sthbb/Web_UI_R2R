      ...
      >
        <span slot="title">
          <div class="diolag-box" :style="{background:$root.theme}">
            <div>{{$t('v_dialog_title_view_grid')}}</div>
          </div>
        </span>
        <SubTable :FUNC_ID="FUNC_ID" :exportTitle="$t('v_dialog_title_view_grid')" :subParams="subParams" GRID_ID="FUNC_POP" ID="GRID_ID" :api="subApi" :delParams="{FUNC_ID:'',GRID_ID:''}" :key="key" :isInitData="true"/>
        <span slot="footer">
...
export default {
  ...
  data() {
    return {
      ...
      dialogVisible:false,
      subApi:{
        select:'FUNCGRID_MANAGEMENT#select',
        add:'FUNCGRID_MANAGEMENT#save',
        update:'FUNCGRID_MANAGEMENT#update',
        delete:'FUNCGRID_MANAGEMENT#delete'
      },
      subParams:{},
      key:0,
      actions:[]
  ...
  methods: {
    ...
    //加载列表数据
    async load(params = this.searchParams,type='FUNC_MANAGEMENT#select',isUpdateSignalID){
      objValueIsNull(params)
      const { modelParaList } = this.$api.parameter
      const res  = await modelParaList(params,type)
      if(type !== 'FUNC_MANAGEMENT#select'){
    ...
    async toDel(data) {
      ...
      this.load(delIds, 'FUNC_MANAGEMENT#delete')
    },
    // 保存
    @saveValidate()
    async toSave(data){
      ...
      this.load(filterData, filterData[0].tableType === 'add' ? 'FUNC_MANAGEMENT#save' : 'FUNC_MANAGEMENT#update',data !== undefined ? this.tableOptions.data[data][this.tableOptions.id] : data)
    },
    //查看详情
    toDetail(data){
      this.subParams = {...searchRequireParams(this.searchForm,this.searchParams),FUNC_ID:this.tableOptions.data[data.rowIndex].FUNC_ID || null,GRID_ID:this.tableOptions.data[data.rowIndex].GRID_ID || null}
      this.key++
