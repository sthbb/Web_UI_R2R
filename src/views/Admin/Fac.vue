...
export default {
  ...
  methods: {
    ...
    //加载列表数据
    async load(params = this.searchParams,type='FAC_MANAGEMENT#select',isUpdateSignalID){
      objValueIsNull(params)
      const { modelParaList } = this.$api.parameter
      const res  = await modelParaList(params,type)
      if(type !== 'FAC_MANAGEMENT#select'){
    ...
    async toDel(data) {
      ...
      this.load(delIds, 'FAC_MANAGEMENT#delete')
    },
    // 保存
    @saveValidate()
    async toSave(data){
      ...
      this.load(filterData, filterData[0].tableType === 'add' ? 'FAC_MANAGEMENT#save' : 'FAC_MANAGEMENT#update',data !== undefined ? this.tableOptions.data[data][this.tableOptions.id] : data)
    }
  }
}
</script>
