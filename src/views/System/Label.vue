...
export default {
  ...
  methods: {
    ...
    //加载列表数据
    async load(params = this.searchParams,type='WEBI18N_MANAGEMENT#select',isUpdateSignalID){
      objValueIsNull(params)
      const { modelParaList } = this.$api.parameter
      const res  = await modelParaList(params,type)
      if(type !== 'WEBI18N_MANAGEMENT#select'){
    ...
    async toDel(data) {
      ...
      this.load(delIds, 'WEBI18N_MANAGEMENT#delete')
    },
    // 保存
    @saveValidate()
    async toSave(data){
      let filterData = data === undefined ? this.tableOptions.data.filter(v=>v.tableType) : [{...this.tableOptions.data[data]}]
      filterData = axiosParams(filterData,this.switchArr,this.numberArr,this.specialkeyArr,true,false)
      this.load(filterData, filterData[0].tableType === 'add' ? 'WEBI18N_MANAGEMENT#save' : 'WEBI18N_MANAGEMENT#update',data !== undefined ? this.tableOptions.data[data][this.tableOptions.id] : data)
    }
  }
}
</script>